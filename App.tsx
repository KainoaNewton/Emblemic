        ...
        if (saved) {
            ...
            const migrated = sorted.map((f: any) => ({
                ...
                config: {
                    ...
                    pixelRounding: f.config.pixelRounding ?? 0
                }
            }));
  ...
  const processExport = async (format: ExportFormat, scope: ExportScope) => {
    ...
    if (format === 'svg') {
        ...
        if (config.mode === 'icon') {
            ...
            const rounding = (config.pixelRounding || 0) / 100; // 0 to 1
            const r = (cellSize / 2) * rounding; // absolute radius

            let pixelContent = '';
            
            // Helper to check neighbor
            const check = (row: number, col: number, color: string) => {
                if (row < 0 || row >= config.pixelGrid.rows || col < 0 || col >= config.pixelGrid.cols) return false;
                return config.pixelGrid.data[row * config.pixelGrid.cols + col] === color;
            };
            ...
            config.pixelGrid.data.forEach((color, i) => {
                if (color) {
                    ...
                    const w = cellSize + 0.1; // Overlap slightly to avoid gaps
                    const h = cellSize + 0.1;
                    
                    if (rounding > 0) {
                        const hasTop = check(row - 1, col, color);
                        const hasBottom = check(row + 1, col, color);
                        const hasLeft = check(row, col - 1, color);
                        const hasRight = check(row, col + 1, color);

                        const tl = (!hasTop && !hasLeft);
                        const tr = (!hasTop && !hasRight);
                        const br = (!hasBottom && !hasRight);
                        const bl = (!hasBottom && !hasLeft);

                        // If no corners rounded, rect
                        if (!tl && !tr && !br && !bl) {
                            pixelContent += `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${color}" />`;
                        } else {
                            // Manual path for rounded corners
                            let d = `M ${x + (tl ? r : 0)} ${y}`;
                            d += ` L ${x + w - (tr ? r : 0)} ${y}`;
                            ...
                            d += ` L ${x} ${y + (tl ? r : 0)}`;
                            if (tl) d += ` A ${r} ${r} 0 0 1 ${x + (tl ? r : 0)} ${y}`;
                            d += ' Z';
                            
                            pixelContent += `<path d="${d}" fill="${color}" />`;
                        }
                    } else {
                        pixelContent += `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${color}" />`;
                    }
    ...
    if (withBg) {
        ...
        if (ctx.roundRect) {
            ...
            // Fallback for older browsers if needed, though roundRect is standard now in modern
    ...
    if (config.mode === 'text') {
        ...
        const rounding = (config.pixelRounding || 0) / 100;
        const r = (cellSize / 2) * rounding;

        // Helper check for canvas export
        const check = (row: number, col: number, color: string) => {
            if (row < 0 || row >= config.pixelGrid.rows || col < 0 || col >= config.pixelGrid.cols) return false;
            return config.pixelGrid.data[row * config.pixelGrid.cols + col] === color;
        };
        ...
        config.pixelGrid.data.forEach((color, i) => {
            if (color) {
                ...
                const x = startX + col * cellSize;
                const y = startY + row * cellSize;
                // Add tiny overlap to prevent subpixel gaps
                     ...
                     <Section title="Pixel Settings">
                        ...
                        <div className="pt-2 space-y-3">
                            <div className="flex items-center justify-between">
                                 ...
                                 <button 
                                    onClick={() => updateConfig({ pixelRounding: config.pixelRounding > 0 ? 0 : 100 })}
                                    className={`w-9 h-5 rounded-full relative transition-colors ${config.pixelRounding > 0 ? 'bg-blue-600' : 'bg-zinc-700'}`}
                                 >
                                    <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-transform duration-200 shadow-sm ${config.pixelRounding > 0 ? 'translate-x-5' : 'translate-x-1'}`} />
                                 </button>
                            </div>
                            {config.pixelRounding > 0 && (
                                <div className="animate-in slide-in-from-top-1 fade-in duration-200 pl-1">
                                    <div className="relative">
                                         <select 
                                            value={[50, 100].includes(config.pixelRounding) ? config.pixelRounding : 100}
                                            onChange={(e) => updateConfig({ pixelRounding: parseInt(e.target.value) })}
                                            className="w-full bg-zinc-900 border border-white/10 rounded-md py-1.5 pl-3 pr-8 text-xs text-zinc-200 focus:outline-none focus:border-blue-500/50 appearance-none cursor-pointer hover:bg-zinc-800 transition-colors"
                                         >
